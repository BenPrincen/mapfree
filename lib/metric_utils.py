...
from lib.utils.reprojection import get_grid_multipleheight
...
def vcre_loss(R, t, Tgt, K0, H=720):
    ...
    eye_coords = (
        torch.from_numpy(eye_coords_glob).unsqueeze(0)[:, :, :3].to(R.device).float()
    )
    ...
    eyes_residual = (
        Rgt.transpose(2, 1) @ eye_coord_tmp
        - 1 * Rgt.transpose(2, 1) @ tgt.transpose(2, 1)
    ).transpose(2, 1)
    ...
    repr_err = ((((uv_gt - uv_pred) ** 2.0).sum(-1) + 1e-6) ** 0.5).mean(-1).view(B, 1)
...
def vcre_torch(R, t, Tgt, K0, reduce=None, H=720, W=540):
    ...
    eyes_residual = (
        torch.linalg.inv(cam2w_gt) @ cam2w_est @ eye_coords.transpose(2, 1)
    ).transpose(2, 1)
    uv_pred = project_2d(eyes_residual[:, :, :3], K0)

    uv_gt[:, :, 0], uv_pred[:, :, 0] = torch.clip(uv_gt[:, :, 0], 0, W), torch.clip(
        uv_pred[:, :, 0], 0, W
    )
    uv_gt[:, :, 1], uv_pred[:, :, 1] = torch.clip(uv_gt[:, :, 1], 0, H), torch.clip(
        uv_pred[:, :, 1], 0, H
    )
...
def print_auc_table(agg_metrics):
    ...
    print(
        "Pose error AUC @ 5/10/20deg: {0:.3f}/{1:.3f}/{2:.3f}".format(
            *auc_pose.values()
        )
    )
    ...
    print(
        "Rotation error AUC @ 5/10/20deg: {0:.3f}/{1:.3f}/{2:.3f}".format(
            *auc_rotation.values()
        )
    )
    ...
    print(
        "Translation angular error AUC @ 5/10/20deg: {0:.3f}/{1:.3f}/{2:.3f}".format(
            *auc_translation_ang.values()
        )
    )

    auc_translation_euc = error_auc(agg_metrics["t_err_euc"], (0.1, 0.5, 1))
    print(
        "Translation Euclidean error AUC @ 0.1/0.5/1m: {0:.3f}/{1:.3f}/{2:.3f}".format(
            *auc_translation_euc.values()
        )
    )

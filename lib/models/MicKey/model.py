...
from lib.gen_utils import precision_recall
from lib.metric_utils import pose_error_torch, vcre_torch
...
class MicKeyTrainingModel(pl.LightningModule):
    def __init__(self, cfg):
        ...
        self.automatic_optimization = (
            False  # This property activates manual optimization.
        )
        self.multi_gpu = True
    ...
    def training_step(self, batch, batch_idx):
        ...
        training_step_ok = self.backward_step(
            batch, outputs, probs_grad, avg_loss, num_its
        )
        self.tensorboard_log_step(
            batch, avg_loss, outputs, probs_grad, training_step_ok
        )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        outputs_metric_ours = pose_error_torch(
            R_ours, t_m_ours, batch["T_0to1"], reduce=None
        )
        ...
        outputs_vcre_ours = vcre_torch(
            R_ours, t_m_ours, batch["T_0to1"], batch["Kori_color0"], reduce=None
        )
    ...
    def backward_step(self, batch, outputs, probs_grad, avg_loss, num_its):
        ...
        invalid_kps0 = (
            torch.isnan(outputs["kps0"].grad).any()
            or torch.isinf(outputs["kps0"].grad).any()
        )
        invalid_kps1 = (
            torch.isnan(outputs["kps1"].grad).any()
            or torch.isinf(outputs["kps1"].grad).any()
        )
        invalid_depth0 = (
            torch.isnan(outputs["depth0"].grad).any()
            or torch.isinf(outputs["depth0"].grad).any()
        )
        invalid_depth1 = (
            torch.isnan(outputs["depth1"].grad).any()
            or torch.isinf(outputs["depth1"].grad).any()
        )
        ...
        if batch["kps0"].requires_grad:
            ...
            torch.autograd.backward(
                (
                    torch.log(batch["final_scores"] + 1e-16),
                    batch["kps0"],
                    batch["kps1"],
                    batch["depth_kp0"],
                    batch["depth_kp1"],
            ...
            torch.autograd.backward(
                (torch.log(batch["final_scores"] + 1e-16)), (probs_grad[0])
            )
    ...
    def tensorboard_log_step(
        self, batch, avg_loss, outputs, probs_grad, training_step_ok
    ):
        ...
        if self.log_store_ims:
            if self.counter_batch % self.log_interval == 0:
                ...
                batch_id = torch.where(outputs["mask_topk"] == 1.0)[0][0].item()

                # Metric pose evaluation
                R_ours, t_m_ours, inliers_ours, inliers_list_ours = (
                    self.e2e_Procrustes.estimate_pose(batch, return_inliers=True)
                )
                outputs_metric_ours = pose_error_torch(
                    R_ours, t_m_ours, batch["T_0to1"], reduce=None
                )
                self.log(
                    "train_metric_pose/ours_t_err_ang",
                    outputs_metric_ours["t_err_ang"].mean().detach(),
                )
                self.log(
                    "train_metric_pose/ours_t_err_euc",
                    outputs_metric_ours["t_err_euc"].mean().detach(),
                )
                self.log(
                    "train_metric_pose/ours_R_err",
                    outputs_metric_ours["R_err"].mean().detach(),
                )
                ...
                self.log(
                    "train_vcre/repr_err", outputs_vcre_ours["repr_err"].mean().detach()
                )

                im_inliers = vis_inliers(inliers_list_ours, batch, batch_i=batch_id)

                im_matches, sc_map0, sc_map1, depth_map0, depth_map1 = (
                    log_image_matches(
                ...
                tensorboard.add_image(
                    "training_matching/best_inliers",
                    im_inliers,
                    global_step=self.log_im_counter_train,
                )
                tensorboard.add_image(
                    "training_matching/best_matches_desc",
                    im_matches,
                    global_step=self.log_im_counter_train,
                )
                tensorboard.add_image(
                    "training_scores/map0",
                    sc_map0,
                    global_step=self.log_im_counter_train,
                )
                tensorboard.add_image(
                    "training_scores/map1",
                    sc_map1,
                    global_step=self.log_im_counter_train,
                )
                tensorboard.add_image(
                ...
                if training_step_ok:
                    try:
                        im_rewards, rew_kp0, rew_kp1 = debug_reward_matches_log(
                            batch, probs_grad, batch_i=batch_id
                        )
                        tensorboard.add_image(
                            "training_rewards/pair0",
                            im_rewards,
                            global_step=self.log_im_counter_train,
                        )
                    except ValueError:
                        print(
                            "[WARNING]: Failed to log reward image. Selected image is not in topK image pairs. "
                        )
    ...
    def on_validation_epoch_end(self):
        ...
        R_threshold = 5
        accepted_poses_ours = (
            aggregated["metric_ours_t_err_euc"].view(-1) < t_threshold
        ...
        _, _, auc_pose = precision_recall(
            inliers=inliers, tp=accepted_poses_ours.detach().cpu().numpy(), failures=0
        )
        ...
        _, _, auc_pose_10 = precision_recall(
            inliers=inliers, tp=accepted_poses_ours.detach().cpu().numpy(), failures=0
        )
        ...
        _, _, auc_vcre = precision_recall(
            inliers=inliers, tp=accepted_vcre_ours.detach().cpu().numpy(), failures=0
        )
        ...
        self.log(
            "val_metric_pose/ours_t_err_ang",
            metric_ours_t_err_ang,
            sync_dist=self.multi_gpu,
        )
        self.log(
            "val_metric_pose/ours_t_err_euc",
            metric_ours_t_err_euc,
            sync_dist=self.multi_gpu,
        )
        self.log(
            "val_metric_pose/ours_R_err", metric_ours_R_err, sync_dist=self.multi_gpu
        )
        ...
        self.log(
            "val_vcre/metric_ours_vcre", metric_ours_vcre, sync_dist=self.multi_gpu
        )

        self.log(
            "val_AUC_pose/prec_pose_ours", prec_pose_ours, sync_dist=self.multi_gpu
        )
        self.log(
            "val_AUC_pose/auc_pose", torch.tensor(auc_pose), sync_dist=self.multi_gpu
        )

        self.log(
            "val_AUC_pose/prec_pose_ours_10",
            prec_pose_ours_10,
            sync_dist=self.multi_gpu,
        )
        self.log(
            "val_AUC_pose/auc_pose_10",
            torch.tensor(auc_pose_10),
            sync_dist=self.multi_gpu,
        )

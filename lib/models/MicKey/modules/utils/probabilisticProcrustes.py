...
class e2eProbabilisticProcrustesSolver:
    ...
    def estimate_pose(self, batch, return_inliers=False):
        ...
        batch_idx_ransac = torch.tile(
            torch.arange(B).view(B, 1), [1, self.num_corr_3d_3d]
        ).reshape(B, self.num_corr_3d_3d)
        ...
        if num_valid_h > 0:
            ...
            for i_b in range(len(max_ind)):
                X_best[i_b], Y_best[i_b] = (
                    dict_corr[it_matches_ids[max_ind[i_b]]]["X"][i_b],
            ...
            for i_ref in range(len(th_ref)):
                inliers = inlier_counting_3d(
                    X_best, Y_best, R, t_metric, th=th_ref[i_ref]
                )
                ...
                R[do_ref], t_metric[do_ref], _ = weighted_procrustes(
                    X_best[do_ref],
                    Y_best[do_ref],
                    use_weights=True,
                    use_mask=True,
            ...
            best_inliers = soft_inlier_counting_3d(
                X_best, Y_best, R, t_metric, th=self.th_inlier
            )
        ...
        if return_inliers:
            if num_valid_h > 0:
                ...
                for i_b in range(len(max_ind)):
                    cor0_best[i_b], cor1_best[i_b] = (
                        dict_corr[it_matches_ids[max_ind[i_b]]]["cor0"][i_b],
                        dict_corr[it_matches_ids[max_ind[i_b]]]["cor1"][i_b],
                    )
                    d0_best[i_b], d1_best[i_b] = (
                        dict_corr[it_matches_ids[max_ind[i_b]]]["d0"][i_b],
                    ...
                    weights_best[i_b] = dict_corr[it_matches_ids[max_ind[i_b]]][
                        "weights"
                    ][i_b]
                ...
                for idx_b in range(len(inliers_idxs)):
                    cor0_inliers = cor0_best[idx_b, inliers_idxs[idx_b] == 1.0]
                    d0_inliers = d0_best[idx_b, inliers_idxs[idx_b] == 1.0]
                    cor1_inliers = cor1_best[idx_b, inliers_idxs[idx_b] == 1.0]
                    d1_inliers = d1_best[idx_b, inliers_idxs[idx_b] == 1.0]
                    score_inliers = weights_best[idx_b, inliers_idxs[idx_b] == 1.0]
                    order_corr = torch.argsort(score_inliers, descending=True)
                    inliers_b = torch.cat(
                        [
                            cor0_inliers[order_corr],
                            cor1_inliers[order_corr],
                            score_inliers[order_corr].unsqueeze(-1),
                            d0_inliers[order_corr],
                            d1_inliers[order_corr],
                        ],
                        dim=1,
                    )
